...
Η εφαρμογή ενσωματώνει μια μηχανή προσωποποιημένων προτάσεων εκδηλώσεων για κάθε συνδεδεμένο χρήστη, βασισμένη στον αλγόριθμο \en{Biased Matrix Factorization (BMF)}. Ο αλγόριθμος αυτός αναλύει το ιστορικό αλληλεπιδράσεων (προβολές και κρατήσεις σε κρατήσεις) που είναι αποθηκευμένο στον πίνακα \en{\texttt{User\_Event\_Interaction}} και προβλέπει το ενδιαφέρον ενός χρήστη για μελλοντικές εκδηλώσεις.
...
\begin{itemize}[leftmargin=*]
    ...
    \item $P_u, Q_i$ : Τα διανύσματα των λανθανόντων παραγόντων (\en{latent factors}) για τον χρήστη και την εκδήλωση αντίστοιχα. Η εσωτερική τους γινόμενη αναπαράσταση αποτυπώνει τη συνάφεια των κρυφών χαρακτηριστικών του χρήστη με την συγκεκριμένη εκδήλωση.
\end{itemize}
...
\vspace{1\baselineskip}
\noindent \textbf{1. Τα πάντα (μαζι με το volume της Βάσης Δεδομένων):}
\begin{lstlisting}[language=bash]
