...
% =========================================================
\section{Εισαγωγή}
Η παρούσα εργασία υλοποιήθηκε στο πλαίσιο του μαθήματος «Τεχνολογίες Εφαρμογών Διαδικτύου» και είχε ως στόχο τη σχεδίαση και ανάπτυξη μιας ολοκληρωμένης διαδικτυακής εφαρμογής διαχείρισης εκδηλώσεων και ηλεκτρονικών κρατήσεων θέσεων, με την ονομασία \en{Planent}.

Η εφαρμογή ακολουθεί την κλασική αρχιτεκτονική \en{web browser / web server}: ο χρήστης αλληλεπιδρά με την υπηρεσία αποκλειστικά μέσω του φυλλομετρητή, ενώ η λογική και τα δεδομένα διαχειρίζονται από ένα \en{Spring Boot} backend που εκθέτει \en{REST API}. Όλη η επικοινωνία πελάτη-εξυπηρετητή πιστοποιείται με χρήση \en{JSON Web Tokens (JWT)} και η εξουσιοδότηση γίνεται βάσει της δραστηριότητας και του ρόλου του χρήστη στο σύστημα.

Στα επόμενα κεφάλαια παρουσιάζονται με συντομία:
\begin{itemize}
    \item Η συνολική αρχιτεκτονική του συστήματος και οι τεχνολογίες που χρησιμοποιήθηκαν (Κεφάλαιο 2).
    \item Οι διαθέσιμες λειτουργίες και οι ρόλοι των χρηστών (Κεφάλαιο 3).
    \item Η σχεδίαση της σχεσιακής βάσης δεδομένων (Κεφάλαιο 4).
    \item Η οργάνωση του \en{backend} και τα κυριότερα \en{REST endpoints} (Κεφάλαιο 5).
    \item Η δομή και τα κυριότερα στοιχεία του \en{frontend} (Κεφάλαιο 6).
    \item Οι μηχανισμοί ασφάλειας που εφαρμόστηκαν (Κεφάλαιο 7).
    \item Η εξαγωγή εκδηλώσεων σε \en{XML/JSON} σύμφωνα με το ζητούμενο \en{DTD} (Κεφάλαιο 8).
    \item Η σχεδίαση του αλγορίθμου συστάσεων (Κεφάλαιο 9).
    \item Οδηγίες εγκατάστασης και εκτέλεσης (Κεφάλαιο 10) και επίλογος (Κεφάλαιο 11).
\end{itemize}
...
Το σύστημα είναι οργανωμένο σε τρία ανεξάρτητα \en{deployable} τμήματα: τη βάση δεδομένων (\en{\textbf{Database}}), το νωτιαίο άκρο (\en{\textbf{Backend}}) και τη διεπαφή χρήστη (\en{\textbf{Frontend}}). Η επικοινωνία μεταξύ των επιμέρους τμημάτων δεν πραγματοποιείται απευθείας μέσω εκτεθειμένου \en{HTTPS}, αλλά βασίζεται στην αρχιτεκτονική των \en{\textbf{containers}}. Συγκεκριμένα, χρησιμοποιείται \en{\textbf{Reverse Proxy}} μέσω του \en{\textbf{Nginx} (Frontend Web Server)}, ο οποίος αναλαμβάνει αποκλειστικά το \en{\textbf{SSL termination}} στην πλευρά του \en{Frontend}, ενώ η εσωτερική δρομολόγηση και ανταλλαγή δεδομένων του \en{\textbf{REST API}} προς το \en{Backend} και τη βάση εκτελείται μέσω απομονωμένων εσωτερικών δικτύων Docker (\en{\textbf{Docker networks}}) τοπικά.

Με αυτή τη σχεδίαση εγγυάται η μέγιστη δυνατή ασφάλεια, καθώς η εξωτερική πρόσβαση επιτρέπεται αποκλειστικά και μόνο μέσω του \en{Frontend} το οποίο προστατεύεται από το πρωτόκολλο \en{\textbf{HTTPS}} χρησιμοποιώντας (\en{self-signed}) \en{\textbf{SSL certificate}}. Παράλληλα, επειδή η επεξεργασία των πιστοποιητικών και η αποκρυπτογράφηση της κίνησης σταματά στον \en{Nginx Reverse Proxy} (\en{SSL termination}), το \en{Backend} απαλλάσσεται πλήρως από το υπολογιστικό βάρος των \en{cryptographic handshakes}, διατηρώντας την απόδοση και την ταχύτητα απόκρισης των \en{endpoints} στο μέγιστο βαθμό.
...
\begin{itemize}[label=\textbullet]
    ...
    \item \en{\textbf{Services:}} Ενσωματώνουν την επιχειρησιακή λογική (\en{business logic}) και τους κανόνες του συστήματος (π.χ. έλεγχος διαθεσιμότητας θέσεων πριν από μια κράτηση). Επίσης, διαχειρίζονται τις συναλλαγές (\en{\textbf{transactions}}) στη βάση δεδομένων μέσω των \en{Repositories}, εξασφαλίζοντας ότι μια ενέργεια είτε θα πετύχει ολοκληρωτικά είτε θα ακυρωθεί αν προκύψει κάποιο σφάλμα (\en{rollback}).
    \item \en{\textbf{Repositories:}} Διεπαφές (\en{interfaces}) του \en{\textbf{Spring Data JPA}} οι οποίες, σε συνδυασμό με το \en{\textbf{Hibernate}} (\en{\textbf{ORM}}), επιτρέπουν την αλληλεπίδραση με τη \en{\textbf{MySQL}} βάση δεδομένων χρησιμοποιώντας αντικείμενα της \en{Java} αντί για χειροκίνητα \en{queries}. Παρέχουν έτοιμες τις βασικές λειτουργίες αποθήκευσης και διαγραφής (\en{\textbf{CRUD}}), καθώς και τη δυνατότητα για δυναμικές αναζητήσεις.
    \item \en{\textbf{Models (Entities):}} Οι κλάσεις της \en{Java} που αντιστοιχούν ακριβώς στους πίνακες της βάσης δεδομένων (όπως \en{User, Event, Booking}). Ορίζουν τη δομή των δεδομένων, τις σχέσεις μεταξύ τους (\en{one-to-many, many-to-many}) και τους περιορισμούς των πεδίων.
    \item \en{\textbf{DTOs (Data Transfer Objects):}} Ξεχωριστά μοντέλα δεδομένων που χρησιμοποιούνται αποκλειστικά για τη μεταφορά πληροφοριών στα \en{Requests} και \en{Responses}. Χρησιμοποιούνται για να μην εκτίθενται τα πραγματικά \en{entities} της βάσης στο διαδίκτυο, επιτρέποντάς μας να φιλτράρουμε την πληροφορία και να στέλνουμε στο \en{Frontend} μόνο τα απαραίτητα πεδία.
\end{itemize}

Η ασφάλεια της εφαρμογής και ο έλεγχος πρόσβασης των χρηστών βασίζονται καθολικά στο \en{\textbf{Spring Security}}, το οποίο αναλαμβάνει τη \en{stateless} αυθεντικοποίηση μέσω \en{JWT} και την εξουσιοδότηση βάσει ρόλων.
...
% =========================================================
    ...
    Η σχεδίαση της σχεσιακής βάσης δεδομένων έγινε με γνώμονα τις λειτουργικές απαιτήσεις της εφαρμογής, αλλά και την απόλυτη συμβατότητα με το ζητούμενο \en{\textbf{Document Type Definition (DTD)}} για την ομαλή εξαγωγή των δεδομένων. Ως Σύστημα Διαχείρισης Βάσεων Δεδομένων (\en{RDBMS}) χρησιμοποιείται η \en{\textbf{MySQL}} με την \textbf{InnoDB} ως \en{storage engine}, η οποία εξασφαλίζει την ακεραιότητα των δεδομένων μέσω ξένων κλειδιών (\en{foreign keys}), μοναδικών περιορισμών (\en{unique keys}) και βασικών κανόνων ελέγχου (π.χ. περιορισμός \en{\texttt{available <= quantity}} στα εισιτήρια). Η σύνδεση του \en{backend} με τη βάση υλοποιείται με το \en{Hibernate} ως \en{ORM} εργαλείο.
    ...
    \begin{table}[H]
        ...
        \bottomrule
        \end{tabularx}
        \caption{Πίνακες της Βάσης Δεδομένων}
    \end{table}

    \subsection{Βασικές Σχέσεις και Περιορισμοί}
    \begin{itemize}
        \item Κάθε εκδήλωση συνδέεται με ακριβώς έναν διοργανωτή (\en{User}) και μπορεί να ανήκει σε πολλές κατηγορίες (\en{\textbf{Many-to-Many}}).
        \item Κάθε εκδήλωση έχει έναν ή περισσότερους τύπους εισιτηρίων. Το άθροισμα των ποσοτήτων ελέγχεται από το \en{\textbf{Event Service}} ώστε να μην υπερβαίνει τη συνολική χωρητικότητα.
        \item Κάθε κράτηση δείχνει σε έναν συγκεκριμένο τύπο εισιτηρίου, ο οποίος διαθέτει το πεδίο \en{available} που μειώνεται ατομικά με κάθε επιτυχμένη κράτηση. Το \en{\textbf{Booking Service}} αναλαμβάνει να κλειδώσει με \en{\textbf{Pessimistic Lock}} τη συγκεκριμένη εγγραφή στη βάση μέχρι να ολοκληρωθεί ή να αποτύχει η κράτηση. Έτσι, αποφεύγεται το \en{data race condition} στην περίπτωση όπου δύο χρήστες επιχειρήσουν να κάνουν κράτηση ταυτόχρονα στο ίδιο εισιτήριο.
        \item Στη διαγραφή εκδήλωσης ορίζονται αντίστοιχα \en{cascade rules} μέσω \en{JPA} (\en{\textbf{CascadeType.ALL} + \textbf{orphanRemoval}}) για τα \en{media}, τις κατηγορίες και τους τύπους εισιτηρίων, εξασφαλίζοντας πως δεν θα υπάρχουν ορφανές εγγραφές.
        \item Τα μηνύματα διαγράφονται οριστικά από τη βάση μόνο εφόσον και οι δύο σημαίες, \en{deletedBySender} και \en{deletedByReceiver}, έχουν τεθεί σε αληθείς από τους αντίστοιχους χρήστες.
    \end{itemize}

    \subsection{Αρχικοποίηση}
    Κατά την πρώτη εκκίνηση του \en{Backend} (\en{\texttt{PlanentApplication.initDatabase}}) δημιουργείται αυτόματα ένας ενσωματωμένος διαχειριστής με \en{\textbf{admin}} όνομα χρήστη και κωδικό πρόσβασης, αν δεν υπάρχει ήδη. Επιπλέον, αρχικοποιούνται αυτόματα και οι βασικές κατηγορίες εκδηλώσεων της πλατφόρμας.
...
% =========================================================
    ...
    Το \en{\textbf{Backend}} εκθέτει ένα \en{\textbf{RESTful API}} με αρχικό \en{context path} \en{\textbf{/api}} που καταναλώνεται αποκλειστικά από το \en{Frontend}. Όλα τα αιτήματα προωθούνται εσωτερικά μέσω του \en{Nginx Reverse Proxy} και, εκτός από τα δημόσια μονοπάτια, απαιτούν την ύπαρξη ενός έγκυρου \en{\textbf{JWT}} στο \en{Authorization Header} ως \en{Bearer token}. Το \en{\textbf{JSON Web Token}} κουβαλάει τις ακόλουθες πληροφορίες (\en{claims}) για τον χρήστη:
    \begin{itemize}
        ...
        \item \en{\textbf{Role} (\texttt{isAdmin}):} Μια \en{boolean} τιμή που δείχνει αν ο χρήστης είναι διαχειριστής ή όχι, η οποία μεταφράζεται σε δικαιώματα \en{\texttt{ADMIN}} ή \en{\texttt{USER}} (\en{Authorities}) μέσα στην εφαρμογή. Αυτό επιτρέπει στο σύστημα να αναγνωρίζει τον ρόλο χωρίς επιπλέον κλήση στη βάση δεδομένων.
    \end{itemize}

    Η διαδικασία αυθεντικοποίησης ελέγχεται αυτόματα σε κάθε αίτηση μέσω του custom φίλτρου \en{\textbf{JwtAuthFilter}}, το οποίο επικυρώνει την υπογραφή του \en{token} και φορτώνει τα στοιχεία του χρήστη στο \en{Security Context} του \en{Spring Boot}.
    ...
    Για τη διαχείριση και την αποθήκευση των εικόνων χρησιμοποιείται η υπηρεσία \en{\textbf{FileSystemStorageService}}, η οποία αποθηκεύει ή διαγράφει αρχεία από τον προκαθορισμένο φάκελο αποθήκευσης στον δίσκο (φάκελος \en{media/}).

    Όταν ανεβαίνει μια νέα εικόνα, η υπηρεσία δημιουργεί έναν ξεχωριστό υποφάκελο με βάση το \en{ID} της εκδήλωσης (\en{\texttt{media/\{eventId\}}}), «καθαρίζει» το όνομα του αρχείου για την αποφυγή επιθέσεων τύπου \en{path traversal} και το μετατρέπει σε μορφή \en{\textbf{.webp}} για εξοικονόμηση χώρου. Μέσω της βιβλιοθήκης \en{\textbf{Thumbnailator}}, η εικόνα περιορίζεται σε μέγιστη διάσταση \en{\textbf{1200x1200px}}, συμπιέζεται στο \en{\textbf{80\%}} της αρχικής της ποιότητας και αφαιρούνται όλα τα ευαίσθητα μεταδεδομένα (\en{\textbf{metadata}}) για λόγους ασφαλείας.

    \subsection{Διαχείριση Σφαλμάτων}
    Όλα τα σφάλματα κανάλιζονται μέσα από έναν \en{\textbf{GlobalExceptionHandler} (@RestControllerAdvice)}, ο οποίος μετατρέπει τα \en{domain exceptions} (\en{ValidationException, ResourceNotFoundException, AccessDeniedException} κ.ά.) σε τυποποιημένες \en{\textbf{RFC 7807} ProblemDetail JSON responses} με κατάλληλο \en{HTTP status} και επεξηγηματικό μήνυμα λάθους.
...
\begin{itemize}[leftmargin=*, itemsep=6pt]
    ...
    \item \textbf{Διαδρομές μόνο για Επισκέπτες (\en{Guest-Only Routes}):} Χρησιμοποιούν το φίλτρο \en{\texttt{GuestRoute}} και επιτρέπουν την πρόσβαση μόνο σε μη συνδεδεμένους χρήστες. Αν κάποιος έχει ήδη συνδεθεί, το σύστημα τον ανακατευθύνει αυτόματα στην αρχική σελίδα.
        \begin{itemize}[leftmargin=0.6cm, itemsep=2pt]
            ...
            \item \en{\texttt{/pending-approval}} : Σελίδα ενημέρωσης για την εκκρεμή έγκριση του διοργανωτή (\en{\texttt{PendingApprovalPage}}).
        \end{itemize}
...
Σε περίπτωση πρόσβασης κάποιας προστατευμένης διαδρομής από έναν χρήστη που δεν έχει πρόσβαση (π.χ. σε \en{edit-event} διαδρομή με event στο οποίο δεν είναι ο διοργανωτής) τότε ο χρήστης μεταβαίνει σε προσαρμοσμένη σελίδα σφάλματος (\en{\texttt{ErrorPage}}) με κωδικό \textbf{\en{404 Unauthorized}}.

Παρόμοια, αν πληκτρολογηθεί μια διεύθυνση που δεν αντιστοιχεί σε καμία από τις παραπάνω ή αφορά \en{event / booking} που δεν υπάρχει τότε εμφανίζεται η σελίδα σφάλματος αλλά με κωδικό \textbf{\en{404 Not Found}}.

\subsection{Επιλεγμένες Σχεδιαστικές Αποφάσεις}
\begin{itemize}[label=\textbullet, leftmargin=*]
    \item \textbf{Διαχωρισμός Μπάρας Πλοήγησης (Conditional Navbar):} Μέσω ελέγχου στο \en{\texttt{location.pathname}}, η εφαρμογή αναγνωρίζει αν ο χρήστης βρίσκεται στον πίνακα διαχείρισης. Αν ο χρήστης είναι \en{admin} και βρίσκεται στη διαδρομή \en{\texttt{/admin}}, το σύστημα εμφανίζει το ειδικό \en{\texttt{AdminNavbar}}, ενώ σε οποιαδήποτε άλλη σελίδα φορτώνει το κανονικό \en{\texttt{Navbar}} της εφαρμογής.
    \item \textbf{Αποκλειστικότητα Διαχειριστή (Admin Interceptor):} Στο κεντρικό component έχει υλοποιηθεί ένα \en{\texttt{useEffect}} το οποίο λειτουργεί ως \en{interceptor}. Αν συνδεθεί χρήστης με ρόλο διαχειριστή, το σύστημα τον ανακατευθύνει αμέσως στο \en{\texttt{/admin}} και του απαγορεύει την πλοήγηση στις σελίδες των απλών χρηστών.
    ...
    \item \textbf{Διαδραστικοί Χάρτες:} Ενσωματώνεται διαδραστικός χάρτης \en{OpenStreetMap} με τη βιβλιοθήκη \en{\textbf{Leaflet}} για την οπτική απεικόνιση των τοποθεσιών των εκδηλώσεων.
    \item \textbf{Αυτόματη Συμπλήρωση Τοποθεσίας:} Χρήση του \en{\texttt{LocationAutocomplete}} component για την αυτόματη εύρεση και συμπλήρωση γεωγραφικών συντεταγμένων κατά τη δημιουργία μιας εκδήλωσης μέσω του \en{\textbf{Geoapify}}.
    \item \textbf{Badge Μη Αναγνωσμένων Μηνυμάτων:} Υπάρχει αυτόματη ενημέρωση του \en{unread badge} στο \en{navbar} για τα νέα μηνύματα.
...
% =========================================================
    ...
    Η κρυπτογραφημένη επικοινωνία εξασφαλίζεται στην πύλη εισόδου της εφαρμογής. Συγκεκριμένα, ο \en{Nginx Reverse Proxy} αναλαμβάνει εξολοκλήρου το \en{SSL termination} χρησιμοποιώντας ένα (\en{self-signed}) \en{SSL certificate}, προστατεύοντας όλη την εξωτερική κίνηση των χρηστών μέσω του πρωτοκόλλου \en{HTTPS}. Μετά την αποκρυπτογράφηση από τον \en{Nginx}, τα αιτήματα δρομολογούνται με ασφάλεια στο \en{Backend} εσωτερικά μέσω του απομονωμένου \en{Docker network} με \en{HTTP} .

    \subsection{Αποθήκευση Κωδικών}
    Για την προστασία των λογαριασμών, όλοι οι κωδικοί πρόσβασης των χρηστών αποθηκεύονται στη βάση δεδομένων αποκλειστικά σε κρυπτογραφημένη μορφή (\en{hashed}) χρησιμοποιώντας τον αλγόριθμο \en{BCrypt} μέσω του \en{\texttt{PasswordEncoder} bean} του \en{Spring Security}. Το σύστημα έχει σχεδιαστεί έτσι ώστε οι κωδικοί να μην εκτίθενται ποτέ σε κανένα \en{DTO} ή \en{HTTP response}.
    ...
    Η διαχείριση των δικαιωμάτων γίνεται με σαφή διαχωρισμό τόσο στο \en{Security Config} όσο και σε επίπεδο \en{Controller}. Συγκεκριμένα, στο \en{\texttt{SecurityFilterChain}} ορίζονται οι καθολικοί κανόνες, όπου τα μονοπάτια \en{\texttt{/auth/**}} και οι αναζητήσεις εκδηλώσεων (\en{\texttt{GET /events/**}}) είναι ελεύθερα προσβάσιμα από όλους (\en{permitAll}) ενώ για τις υπόλοιπες ενέργειες απαιτείται αυθεντικοποίηση, με τον έλεγχο των δικαιωμάτων να γίνεται με τη χρήση του annotation \en{\texttt{@PreAuthorize}} στους \en{Controllers}.

    \subsection{Προστασία από Spam / Σπατάλη Πόρων}
    \begin{itemize}[leftmargin=*]
    \item \en{\textbf{CORS}:} Αυστηρά ρυθμισμένο στο \en{Spring Security} ώστε να δέχεται αιτήματα αποκλειστικά από το \en{URL} του \en{frontend}, μπλοκάροντας κακόβουλες εξωτερικές κλήσεις.
    \item \en{\textbf{XSS Protection}:} Η βιβλιοθήκη \en{React} προστατεύει την εφαρμογή από επιθέσεις \en{Cross-Site Scripting} κάνοντας αυτόματα \en{escape} οποιαδήποτε τιμή απεικονίζεται στο περιβάλλον εργασίας. Με αυτόν τον τρόπο, κακόβουλα κείμενα ή εντολές κώδικα που τυχόν προσπαθήσει να εισάγει ένας χρήστης αντιμετωπίζονται ως απλό κείμενο και δεν εκτελούνται ποτέ στον φυλλομετρητή.
    \item \item \en{\textbf{Stateless Sessions}:} Η εφαρμογή δεν διατηρεί \en{session} στον \en{server}, αποτρέποντας έτσι επιθέσεις τύπου \en{session fixation} και μειώνοντας τη χρήση μνήμης.
    \item \en{\textbf{Validation}:} Κάθε εισερχόμενο αίτημα ελέγχεται στην πλευρά του \en{Backend} με τη χρήση του \en{Jakarta Bean Validation} (\en{\texttt{@NotBlank, @Min, @Email}} στα \en{DTOs}), απορρίπτοντας αμέσως άκυρα δεδομένα πριν αυτά φτάσουν στη βάση.
    \end{itemize}
...
% =========================================================
    ...
    Ο διαχειριστής της εφαρμογής έχει τη δυνατότητα να εξάγει όλες τις καταχωρημένες εκδηλώσεις του συστήματος σε δομημένη μορφή αρχείου μέσω του \en{endpoint} \en{\texttt{/admin/export/events}} ορίζοντας την παράμετρο \en{\texttt{?format=xml}} ή \en{\texttt{?format=json}}. Η υλοποίηση αυτή βασίζεται σε ξεχωριστά, ειδικά σχεδιασμένα \en{Export DTOs} (\en{\texttt{EventsExport}}), τα οποία γίνονται \en{serialize} με τη χρήση των \en{\texttt{XmlMapper}} και \en{\texttt{JsonMapper}} της βιβλιοθήκης \en{Jackson}.
...
% =========================================================
    ...
    Η εφαρμογή ενσωματώνει μια μηχανή προσωποποιημένων προτάσεων εκδηλώσεων για κάθε συνδεδεμένο χρήστη, βασισμένη στον αλγόριθμο \en{Biased Matrix Factorization (BMF)}. Ο αλγόριθμος αυτός αναλύει το ιστορικό αλληλεπιδράσεων (προβολές και κρατήσεις) που είναι αποθηκευμένο στον πίνακα \en{\texttt{User\_Event\_Interaction}} και προβλέπει το ενδιαφέρον ενός χρήστη για μελλοντικές εκδηλώσεις.
    ...
    \begin{itemize}[leftmargin=*]
    \item $\mu$ : Η καθολική τάση του συστήματος (\en{global bias}), η οποία είναι αποθηκευμένη στον πίνακα \en{\texttt{Recommendation\_Config}}.
    \item $b_u$ : Η προσωπική τάση του χρήστη (\en{user bias}), που αντικατοπτρίζει πόσο αυστηρός ή ελαστικός είναι γενικά στις επιλογές του.
    \item $b_i$ : Η τάση της εκδήλωσης (\en{item bias}), που δείχνει τη γενική δημοτικότητα της συγκεκριμένης εκδήλωσης στην πλατφόρμα.
    \item $P_u, Q_i$ : Τα διανύσματα των λανθανόντων παραγόντων (\en{latent factors}) για τον χρήστη και την εκδήλωση αντίστοιχα, με μέγεθος $K = 3$. Η εσωτερική τους γινόμενη αναπαράσταση αποτυπώνει τη συνάφεια των κρυφών χαρακτηριστικών του χρήστη με το προφίλ της εκδήλωσης.
    \end{itemize}

    \subsection{Εκπαίδευση και Υπερπαράμετροι}
    Η εκπαίδευση του μοντέλου πραγματοποιείται μέσω της μεθόδου της Στοχαστικής Κλιμάκωσης Δυναμικού (\en{Stochastic Gradient Descent - SGD}). Κατά την εκκίνηση, οι λανθάνοντες παράγοντες αρχικοποιούνται με τυχαίες μικρές τιμές. Οι παράμετροι εκπαίδευσης ελέγχονται από τον πίνακα ρυθμίσεων και έχουν οριστεί με βάση τις βέλτιστες δοκιμές:
    \begin{itemize}[leftmargin=*]
    \item \textbf{Εποχές (\en{Epochs}):} Το μοντέλο εκτελεί $60$ πλήρεις κύκλους εκπαίδευσης πάνω στα δεδομένα για τη βελτιστοποίηση των τιμών.
    \item \textbf{Ρυθμός Μάθησης (\en{Learning Rate}):} Ορίστηκε σταθερά στην τιμή $0.025$ για την ομαλή σύγκλιση του αλγορίθμου χωρίς αποκλίσεις.
    \item \textbf{Κανονικοποίηση (\en{Regularization}):} Εφαρμόζεται συντελεστής $0.03$ για την αποφυγή του φαινομένου της υπερπροσαρμογής (\en{overfitting}), εξασφαλίζοντας ότι οι συστάσεις θα παραμείνουν γενικευμένες και ποιοτικές.
    \end{itemize}

    Οι τελικές τιμές των διανυσμάτων αποθηκεύονται στους πίνακες \en{\texttt{User\_Recommendation\_Vector}} και \en{\texttt{Event\_Recommendation\_Vector}}, επιτρέποντας στο \en{Backend} να παράγει άμεσα και σε πραγματικό χρόνο τις σελιδοποιημένες προτάσεις κατά την κλήση των δημόσιων \en{endpoints}.
...
\begin{itemize}[label=\textbullet]
    \item \en{Java 21 (JDK)}
    \item \en{Maven 3.9+} (ή χρήση του \en{mvnw})
    \item \en{Node.js 20+} και \en{npm 10+}
    \item \en{MySQL Server 8+}
\end{itemize}
