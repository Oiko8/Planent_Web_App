...
Κατά την ανάπτυξη της εφαρμογής, εστιάσαμε στην εξοικείωση με ένα σύγχρονο \en{full-stack tech stack} και στην επίλυση πραγματικών προκλήσεων, όπως η διαχείριση ταυτόχρονων χρηστών και η δικτυακή ασφάλεια. Στόχος μας ήταν να χτίσουμε μια ολοκληρωμένη πλατφόρμα, γρήγορη στις αποκρίσεις της, η οποία εγγυάται την ακεραιότητα των δεδομένων.
\newpage
...
\begin{itemize}
    \item \textbf{Σχέσεις Πινάκων:} Η διασύνδεση των δεδομένων βασίζεται σε περιορισμούς ξένων κλειδιών (\en{Foreign Keys}). Οι σχέσεις ManyToMany, όπως αυτή μεταξύ εκδηλώσεων και κατηγοριών, επιλύονται με τη χρήση του πίνακα ένωσης \en{\texttt{Event\_Category}} (\en{Many-to-Many}).
...
\subsubsection{Διαχείριση και Βελτιστοποίηση SQL Ερωτημάτων}
\begin{itemize}[leftmargin=*]
    \item \textbf{Αντιμετώπιση του N+1 με Batch Fetching:} Λόγω της χρήσης \en{Lazy Fetching} στις \en{OneToMany} συσχετίσεις των \en{Entities} (όπως οι κατηγορίες, τα εισιτήρια και τα πολυμέσα στις εκδηλώσεις), αρχικά εμφανίστηκε το πρόβλημα των $N+1$ ερωτημάτων κατά την ανάκτηση των δεδομένων αυτών. Το ζήτημα αυτό λύθηκε καθολικά στο \en{\texttt{application.properties}} με την ιδιότητα \en{\texttt{default\_batch\_fetch\_size=50}}, αναγκάζοντας το \en{Hibernate} να ομαδοποιεί τα lazy queries σε batches των 50 εγγραφών. Έτσι, για 50 events χρειάζονται μόνο 1 αρχικό query για τα ίδια τα events και 3 επιπλέον batch queries για τα categories, ticket types και media τους, μειώνοντας δραστικά τα database hits απο 151 σε μόλις 4.
    \item \textbf{Eager Fetching και Πολυεπίπεδα Joins:} Σε endpoints όπου χρειαζόμαστε άμεσα τις συσχετισμένες πληροφορίες, παρακάμπτουμε το lazy loading χρησιμοποιώντας ρητά \en{JOIN FETCH} και \en{LEFT JOIN FETCH} μέσα σε custom JPQL queries των repositories. Για παράδειγμα, στο \en{BookingRepository} πραγματοποιούνται πολυεπίπεδα fetches (\en{\texttt{FROM Booking b JOIN FETCH b.ticketType tt JOIN FETCH tt.event}}), επιτρέποντας στο σύστημα να επιστρέφει την κράτηση, τον τύπο εισιτηρίου και την αντίστοιχη εκδήλωση με ένα και μοναδικό βελτιστοποιημένο query στη βάση, αντί για να χρειάζεται έπειτα η Hibernate να εκτελεί ξεχωριστά ερωτήματα για να τα κάνει fetch, αφού θα τα χρειαστούμε.
    \item \textbf{Lightweight Queries:} Για την εκτέλεση γρήγορων ελέγχων (όπως η επαλήθευση δικαιωμάτων ή η ύπαρξη εγγραφών), αποφεύγεται η πλήρης φόρτωση ολόκληρων των entities στη μνήμη. Στο \en{EventRepository}, για παράδειγμα, η μέθοδος \en{\texttt{findOrganizerIdById}} εκτελεί ένα στοχευμένο query που επιστρέφει αποκλειστικά το \en{ID} του διοργανωτή, επιτρέποντας στο service να κάνει ακαριαία τον έλεγχο ιδιοκτησίας, χωρίς να σπαταλάει μνήμη για να σηκώσει ολόκληρο το αντικείμενο του \en{User}.
\end{itemize}
...
\end{itemize}
Η διαδικασία αυτή έβγαλε ιδανικό \en{config} το εξής: 3 λανθάνοντες παράγοντες, 0.025 learning rate, 0.03 regularization και 60 epochs. Οι ρυθμίσεις αυτές μεταφέρθηκαν αυτούσιες στον πίνακα \en{\texttt{Recommendation\_Config}} της βάσης δεδομένων για τα μετέπειτα \en{trainings} με αληθινούς χρήστες και εκδηλώσεις στην εφαρμογή.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Caching σε HashMaps:} Πριν ξεκινήσουν οι επαναλήψεις, όλες οι υπάρχουσες εγγραφές των χρηστών και των εκδηλώσεων φορτώνονται από τη βάση σε τοπικά \en{HashMaps}. Αυτό επιτρέπει ακαριαίες αναζητήσεις (\en{O(1)} lookups) και ενημερώσεις των διανυσμάτων στη μνήμη, αποφεύγοντας χιλιάδες ενδιάμεσα queries. Ωστόσο, η προσέγγιση αυτή αποτελεί ένα \en{trade-off}: ενώ είναι ιδανική για το τρέχον μικρό μέγεθος της πλατφόρμας εκμηδενίζοντας τα \en{I/O bottlenecks}, δεν θα ήταν βιώσιμη αν το σύστημα μεγάλωνε απότομα και αποκτούσε εκατομμύρια χρήστες, καθώς η φόρτωση όλων των vectors στη \en{RAM} θα προκαλούσε σφάλματα \en{Out-of-Memory}, απαιτώντας αλλαγή σε \en{batch-based} εκπαίδευση.
...
Παράλληλα, το query αντιμετωπίζει δυναμικά τις περιπτώσεις ελλιπών δεδομένων μέσω της SQL συνάρτησης \en{COALESCE}, η οποία λειτουργεί ως δικλείδα ασφαλείας (\en{fallback}) σε δύο κρίσιμα σενάρια:
\begin{itemize}[leftmargin=*]
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Μόνο Containers \& Images (χωρίς το volume της Βάσης Δεδομένων):}
    \begin{lstlisting}[language=bash]
    docker compose down --rmi all
    \end{lstlisting}

    \item \textbf{Τα πάντα (μαζί με το volume της Βάσης Δεδομένων):}
    \begin{lstlisting}[language=bash]
